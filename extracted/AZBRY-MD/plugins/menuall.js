// plugins/menuall.js
// Azbry-MD • Full Elegant Theme Menu (Header disamakan dengan menu utama)
...
// urutan prioritas dari ATAS → BAWAH tampilan
// (artinya info akan jadi paling bawah)
...
// header sama persis kayak menu utama
...
// fungsi bantu buat nyusun urutan kategori dinamis
function buildOrderedCategories() {
  ...
  const all = Array.from(tagsInUse)
  const want = PRIORITY_ORDER_TOP_TO_BOTTOM.filter(t => all.includes(t))
  const others = all.filter(t => !want.includes(t))
  return [...others, ...want]
}
...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    const categories = buildOrderedCategories()
