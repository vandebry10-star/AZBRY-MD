...
async function sendWait(conn, m) {
  try {
    if (fs.existsSync(WAIT_STK)) {
      // KIRIM STIKER SAJA
      await conn.sendMessage(m.chat, { sticker: { url: WAIT_STK } }, { quoted: m })
      return
    }
  } catch {}
  // Fallback: kalau file stiker tidak ada, baru kirim teks
        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    // --- FORCE DEFAULT LIMIT 10 (dan betulin jika ada yang timpa ke 100) ---
if (user.limit == null || Number.isNaN(user.limit)) user.limit = 5
if (Number(user.limit) === 100) user.limit = 5
                    // --- DEBUG: lacak siapa yang nulis limit ---
try {
  let __limit = user.limit
  Object.defineProperty(user, 'limit', {
    get() { return __limit },
    set(v) {
      const val = Number(v)
      if (val === 100) {
        console.log('[LIMIT-SET]', m.sender, '->', val, '\n', (new Error()).stack.split('\n')[2]?.trim())
      }
      __limit = val
    },
    enumerable: true,
...
│
│ 🔁 Beli ulang limit dengan perintah:
│ ➤ *${usedPrefix}sewa* atau chat *${usedPrefix}owner*
│ Biasanya kalau owner baik bakal dikasih limit gratis, coba hubungi dia 🔥
│ 🎁 Join Grup Azbry-MD untuk bonus limit
│ https://chat.whatsapp.com/CYa5CRl8K1lAK4NNCYmHJe?mode=wwt
╰───────────────────────
`, m)
...
if (!isPrems && plugin.limit) {
  const usedLimit = plugin.limit === true ? 2 : Math.max(1, Number(plugin.limit) || 0)
    // 🔽 Tambahkan ini untuk potong limit real
  let user = global.db.data.users[m.sender]
  if (user) user.limit = Math.max(0, (user.limit || 0) - usedLimit)
  await m.reply(
    `*Limit digunakan:* [ -${usedLimit} ]\n\nSelalu cek sisa limit kamu dengan cara *_.limit_*`
  )
}
                   }
                    break
